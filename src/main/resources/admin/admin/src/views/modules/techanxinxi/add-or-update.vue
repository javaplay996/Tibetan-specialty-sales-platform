    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="特产类型" prop="techanleixing">
          <el-select :disabled="ro.techanleixing" v-model="ruleForm.techanleixing" placeholder="请选择特产类型">
            <el-option
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.techantupian" label="特产图片" prop="techantupian">
          <file-upload
          tip="点击上传特产图片"
          action="file/upload"
          :limit="3"
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.techantupian" label="特产图片" prop="techantupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.techantupian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="特产介绍" prop="techanjieshao">
                <editor 
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.techanjieshao" label="特产介绍" prop="techanjieshao">
                    <span v-html="ruleForm.techanjieshao"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.techanjieshao = true;
          ...
          if(o=='techantupian'){
            this.ruleForm.techantupian = obj[o];
	...
	this.ruleForm.techanjieshao = this.ruleForm.techanjieshao.replace(reg,'../../../springbootoqf8h/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
	...
	if(this.ruleForm.techantupian!=null) {
		this.ruleForm.techantupian = this.ruleForm.techantupian.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `techanxinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.techanxinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
	...
	this.ruleForm.techantupian = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
